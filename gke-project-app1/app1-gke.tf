...
locals {
  envs = {
    dev = {
      ...
      subnetwork                 = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[1]}" #refer to node subnet
      ip_range_pods              = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[1][0]}"
      ip_range_services          = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[1][1]}"
...
prod = {
      ...
      network                    = "${data.terraform_remote_state.projects.outputs.app1_gke_network}"
      subnetwork                 = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[2]}" #refer to node subnet
      ip_range_pods              = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[2][0]}"
      ip_range_services          = "${data.terraform_remote_state.projects.outputs.app1_gke_subnets[2][1]}"
      http_load_balancing        = false
      network_policy             = false
      horizontal_pod_autoscaling = true
      filestore_csi_driver       = false
